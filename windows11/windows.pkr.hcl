...
build {
  ...
  //     "--extra-vars",
  //     "ansible_command_timeout=1001",
  //     "--extra-vars",
  //     "ansible_winrm_read_timeout_sec=1002"
  //   ]
  // }

  // provisioner "file" {
  //   source      = "c:\\windows\\temp\\inventory"
  //   destination = "${path.cwd}/ansible/inventory"
  //   direction   = "download"
  // }

  // provisioner "shell-local" {
  // environment_vars  = ["WINRMPASS=${build.Password}"]
  // command = " ansible-playbook -vvv -i ${path.cwd}/ansible/inventory -e@${path.cwd}/ansible/vars.yml -e ansible_password=\"$WINRMPASS\" ${path.cwd}/ansible/main.yml"
  // command = " ansible-playbook -vvv -i ${path.cwd}/ansible/inventory  -e ansible_password=\"$WINRMPASS\" -e@${path.cwd}/ansible/vars.yml ${path.cwd}/ansible/main.yml"
  //}
